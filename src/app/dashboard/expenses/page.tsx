...
const CATEGORIES = [
  "Software & Tools", "Marketing", "Salaries", "Office & Rent",
  "Travel", "Hardware", "Taxes & Fees", "Other",
];

const CATEGORY_STYLES: Record<string, string> = {
  "Software & Tools": "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-700/50",
  "Marketing":        "bg-fuchsia-50 dark:bg-fuchsia-900/30 text-fuchsia-700 dark:text-fuchsia-300 border-fuchsia-200 dark:border-fuchsia-700/50",
  "Salaries":         "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-700/50",
  "Office & Rent":    "bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-700/50",
  "Travel":           "bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-700/50",
  "Hardware":         "bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 border-violet-200 dark:border-violet-700/50",
  "Taxes & Fees":     "bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-700/50",
...
export default function ExpensesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) { await fetchExpenses(); setModalOpen(false); }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            ...
            record{filtered.length !== 1 ? "s" : ""} totaling{" "}
            <strong className="text-blue-600 dark:text-blue-400 font-semibold text-lg">
          ...
          <button
            ...
            className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-all border ${
              filterCategory === cat
                ...
                : "bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-slate-700 hover:border-gray-300 dark:hover:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700"
            }`}
          >
            {cat}
          </button>
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-gray-200 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-700/30">
                  <th className="px-6 py-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">Category</th>
                  <th className="px-6 py-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">Date</th>
                  <th className="px-6 py-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">Notes</th>
                  <th className="px-6 py-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs text-right">Amount</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400 max-w-xs truncate">
                      {expense.notes ? (
                        ...
                        <span className="text-gray-300 dark:text-gray-600 italic">No notes added</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          ...
                          className="p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors disabled:opacity-40"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {error && (
                  <div className="p-3 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-xl text-sm font-medium border border-red-100 dark:border-red-800/50">
                    {error}
                  </div>
                )}
